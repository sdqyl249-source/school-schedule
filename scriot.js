...
function deleteAnnouncement(key) {
    // 1. التحقق من صلاحية الأدمن محلياً
    if (localStorage.getItem("userLevel") !== "admin") {
        alert("❌ عذراً، غير مصرح لك بالحذف!");
        return;
    }

    // 2. طلب تأكيد الحذف من المستخدم
    if (confirm("⚠️ هل أنت متأكد من حذف هذا الإعلان نهائياً؟")) {
        
        // 3. الحذف المباشر
        // ملاحظة: بما أنك استخدمت القواعد الجديدة التي تسمح بالحذف للمفتاح 'ahmed' 
        // أو عند عدم وجود بيانات (حذف)، سيعمل هذا الكود فوراً.
        database.ref('announcements/' + key).remove()
            ...
            .catch((error) => {
                // في حال وجود خطأ في الاتصال أو الصلاحيات
